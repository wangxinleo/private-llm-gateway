...
- [ ] **Regex with greedy quantifiers?** Before writing `+` / `*` / `{n,}` patterns that scan untrusted/bulk text (base64, tokens, markdown, ANSI), check for catastrophic backtracking
  - Greedy quantifier + rare following token = O(n²) on no-match inputs
  - Fix with lookbehind boundary, tightened character class, or length cap
  - See [Regex Performance Guide](./regex-performance-guide.md)
  - Time it on 1MB+ real-world input before shipping; add a <200ms regression test
