...
### EventSource Admin Streams

Use this contract for browser `EventSource` connections to admin-only SSE endpoints.

1. `EventSource` cannot send custom headers. If the backend endpoint is admin-protected, pass the admin key in the stream URL only for endpoints that explicitly allow query-key auth.
2. Create/recreate the `EventSource` inside an effect that depends on the auth key. Do not create the stream before the key is loaded.
3. On cleanup, close the current `EventSource`, clear reconnect timers, and mark the connection offline.
4. On each live event, either merge the row directly when the current page/filter can be updated locally, or refetch the current query when filters/pagination are active.

```typescript
export function useAdminEventStream(adminKey: string | null) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      if (closed) return;
      const source = new EventSource(`/api/admin/audit/stream?key=${encodeURIComponent(adminKey)}`);
...
| Wrong | Correct |
| ----- | ------- |
| Open `EventSource('/api/admin/audit/stream')` while auth is stored in `x-admin-key` fetch headers | Open `EventSource('/api/admin/audit/stream?key=...')` only after `adminKey` is loaded |
| `useEffect(..., [])` for an authenticated stream | Depend on `adminKey` so login/logout reconnects correctly |
| Ignore events while filters are active | Refetch the current query so filtered views stay live |
