...
## Admin SSE Authentication Contract

### Scope / Trigger

Use this contract for admin-only Server-Sent Events endpoints, especially audit-log live refresh.

This project requires a single admin authentication transport: the `x-admin-key` request header. Do not pass `ADMIN_KEY` in URLs.

### Signatures

```typescript
checkAdminAuth(req: Request): NextResponse | null

authedFetch("/api/admin/audit/stream", { signal: abortController.signal })
```

### Contracts

- All admin routes, including SSE routes, authenticate with the `x-admin-key` request header only.
- Admin routes must not accept `?key=...`, `?adminKey=...`, or other query-parameter credentials.
- Browser `EventSource` cannot send custom headers, so admin SSE clients must use `fetch` streaming instead of `EventSource`.
- Frontend stream clients must use the shared authenticated fetch path so auth remains consistent with JSON admin APIs.
- Stream clients must abort the fetch on unmount/logout and reconnect only while the admin key is still present.
...
| ---- | --------------- |
| `ADMIN_KEY` missing | `503 { error: "admin_not_configured" }` |
| No `x-admin-key` header | `401 { error: "unauthorized" }` |
| Wrong `x-admin-key` header | `401 { error: "unauthorized" }` |
| Correct `x-admin-key` header | authenticated |
| Correct key supplied only as query parameter | `401 { error: "unauthorized" }` |
| Wrong header plus correct query parameter | `401 { error: "unauthorized" }` |

### Good/Base/Bad Cases

```typescript
// GOOD: Admin SSE uses the same header-based auth helper as every admin route.
...
// GOOD: Browser client uses fetch streaming so x-admin-key can be sent.
const res = await authedFetch("/api/admin/audit/stream", { signal });
const reader = res.body?.getReader();

// BASE: Normal admin JSON endpoints use the same helper.
const authError = checkAdminAuth(request);

// BAD: EventSource cannot send x-admin-key and would require URL credentials.
new EventSource(`/api/admin/audit/stream?key=${adminKey}`);
```

### Tests Required

- `checkAdminAuth` accepts only the `x-admin-key` header and rejects matching query credentials.
- SSE route accepts `x-admin-key`, rejects missing/query-only keys, returns the SSE headers above, and forwards broadcast events to subscribers.
- Frontend stream client does not construct URLs containing `ADMIN_KEY` or `?key=`.

### Wrong vs Correct

| Wrong | Correct |
| ----- | ------- |
| Putting `ADMIN_KEY` in an SSE URL query parameter | Use `fetch` streaming with `x-admin-key` |
| Mixing query auth for SSE and header auth for JSON admin APIs | Use header auth for every admin API |
| Using browser `EventSource` for authenticated admin streams | Use `fetch` plus `ReadableStream` parsing |
| Returning default cache headers for SSE | Return `no-cache, no-transform` and disable proxy buffering |
