...
> **Warning**: Response-side content-type judgments must normalize (lowercase + trim) before matching. Upstreams do not guarantee case: a `Text/Event-Stream` response failed the case-sensitive `includes("text/event-stream")` check and fell into the buffered path (`await upstream.text()`), hanging until the whole stream completed (streaming UX lost); an `Application/Octet-Stream` response failed the case-sensitive binary check and got UTF-8-decoded, corrupting the body (12 B → 18 B, irreversible; 2026-09-23 probe). `normalizeContentType` / `isSseContentType` / `isBinaryContentType` in `route.ts` are the single entry points — never inline raw `includes` checks on the header again. Request-side judges (`isMultipart`, `isJsonContentType`) already normalize; regression tests live in `response-content-type.test.ts`.

> **Warning**: The buffered response path must stay size-capped. `finalizeUpstream`'s non-SSE/non-binary branch reads the whole response and runs `restoreText` + `analyzeResponse` synchronously — uncapped, a multi-MB response occupies the event loop for every session in the process (2026-09-23 probe: with a 1 KB limit, a 4 KB response was fully read and restored; a never-ending overseized JSON response hung the request until the stream finished). `readBodyCapped` stops at `RUNTIME.maxBodyBytes` (shared with the request side, no separate knob) and rebuilds the buffered prefix + remaining upstream stream into a byte-identical passthrough; the skip MUST leave a trace — `restore_skipped` audit signal (`reason`/`bytes`/`limit` only, never body text) plus a `log.warn`, otherwise users see bare placeholders with no clue why. The declared-`content-length` fast-skip applies only when the encoding is `none`/`zstd`; `decoded` (gzip/br via undici) must not be fast-skipped because wire size ≠ decoded size. Regression tests live in `response-size-guard.test.ts`.

> **Warning**: Any passthrough of an undici-decoded body must drop the wire `content-length`, not just `content-encoding`. For `gzip`/`deflate`/`br` undici hands us decompressed bytes while the header still carries the *compressed* size; a client honoring it truncates silently (2026-09-23 E2E: `/big-gzip`, 2.1 MB decoded body with wire `content-length: 2117`, delivered 2117 B — prefix-correct, zero errors). `reemitUpstream` is the single choke point: it deletes `content-length` whenever `classifyContentEncoding` says `decoded` (covers size-skip, binary, legacy zero-copy), while `none`/`zstd`/`unknown` keep theirs because there the body is the wire bytes. The `readBodyCapped` oversized branch re-wraps a stream, so it inherits this fix only through `reemitUpstream` — never construct a passthrough `Response` from `upstream.headers` without it.

> **Warning**: Response restore observability must be single-detector and single-count. `findResidualPlaceholders` (`src/scanner/placeholder-scan.ts`) is the only residual counter — it feeds both the `response_poison.placeholder_residual` signal and the `restore_unresolved` audit column, so the two values must agree; counting only strict `TAG_RE` was narrower than the repair surface (maskit lesson: page full of unresolved tags, event log reports 1). Counting happens only when the request actually issued placeholders (non-empty registry) and is persisted via `recordRestoreStats` (UPDATE + `audit_update` SSE merge-by-id, never a second insert row); NULL columns mean "no restore pass ran" (zero-copy/binary/legacy/size-skip) and must not be materialized as 0. `LOOSE_RX` accepts 1-2 braces, optional whitespace *inside* braces and lowercase labels, but whitespace must never be part of a bare-token match — `\s?` outside the brace group swallowed the separator space ("and PHONE_x" → "and" + value), caught by `restore.test.ts`; `restore-stress.test.ts` (130 values) plus `restore-observability.test.ts` pin the counts.

> **Warning**: Tag-shape regexes must keep bounded label quantifiers — never `[A-Za-z][A-Za-z0-9_]*_...`. The unbounded form backtracks O(n²) on whitespace-free runs: every start position scans to the end before failing on the missing `_` (2026-09-23 E2E: a 1 MB lowercase word-run response froze the whole event loop for minutes — `sample` showed `StringPrototypeReplace → RegExpExecMultiple`, micro-bench 40 KB → 581 ms doubling ×4 per 2× input; `edge-at?ch=A` proved the pre-existing uppercase-only hazard too). `LOOSE_CORE` (`mask-tag.ts`) and `TAG_CORE` (`placeholder-scan.ts`) are both `[A-Za-z][A-Za-z0-9_]{0,MAX_SHORTCODE_LEN}_...`; real shortcodes max out at `PRIVATE_KEY` (11), so the bound costs nothing — and the two shapes must stay in lockstep, or the counting surface narrows below the repair surface. `benchmarks/regex-linear-guard.test.ts` pins both scans at 100 KB word-runs; `TAG_PARTIAL_RE`/`TAG_RE` are exempt only because their call sites bound the input (`pending.length <= MAX_TAG_LEN`) or their class cannot cross `{`.

> **Warning**: Every upstream failure must be traced, and the trace must distinguish "died before responding" from "died mid-response". `describeUpstreamError` / `formatUpstreamError` (`src/proxy/error-trace.ts`) are the single diagnostic formatters: `resp=0` (connection dead before headers — typical symptom of a reused idle connection closed by the upstream) vs `resp=1` (headers received, body/stream then broke); `err` is the cause's name (the outer undici `TypeError: fetch failed` is useless), `code` is looked up on the cause first then on the error itself, plus `req`/`out` bytes and `ms`. The trace goes to a `log.warn` prefix and an `upstream_error` (HIGH) audit signal — metadata only, never into the client-facing 502 body (hostname/port/none of the error message; `route-upstream-error.test.ts` pins zero leakage). Two traps: (1) a mid-stream abort in `streaming.ts` must record the trace *before* `controller.error(err)` (it was fully silent — no log, no signal; client-visible truncation semantics stay unchanged); (2) `return someAsyncFn()` inside a try/catch does NOT route the returned promise's rejection through the catch — `finalizeUpstream` returned that way, so buffered-path body failures (`resp=1`), zstd decode errors and restore/persist errors escaped as uncaught rejections (500, zero audit trail) instead of the intended 502; the call must stay `return await finalizeUpstream(...)`. Zero-copy passthrough bodies (binary/unknown encoding/bypass non-SSE) are pumped by the consumer and stay out of this diagnostic surface — a documented boundary, not an oversight.

> **Warning**: A client-side disconnect is not an upstream failure — never let it write `upstream_error`. When the consumer cancels, the pending `pull` loop's `reader.read()` resolves `done`, and the subsequent `controller.close()`/`enqueue` throws `ERR_INVALID_STATE`; the `streaming.ts` catch treated that as a mid-stream abort (2026-09-23 E2E: `curl --max-time` produced a HIGH `upstream_error` `{err:"TypeError",code:"ERR_INVALID_STATE"}` with zero upstream involvement). The wrapper stream's `cancel(reason)` sets a `clientCancelled` flag (and cancels the upstream reader); the catch branches on it to debug-log only, while genuine upstream aborts keep `log.warn` + `recordAbort`. Note the partial-pass side effect is intended: `analyzer.finish()` may still run before the throw, so restore counts/residual signals for the delivered prefix are persisted — the audit row is not NULL and the only signal must be `response_poison`, not `upstream_error`. `streaming-client-cancel.test.ts` pins both directions; the debug line needs `DEBUG=true` (the string), `DEBUG=1` stays at info level.
