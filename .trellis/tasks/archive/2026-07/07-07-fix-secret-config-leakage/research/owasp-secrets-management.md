[Skip to content](#secrets-management-cheat-sheet)
...
* [2 General Secrets Management](#2-general-secrets-management) 
  ...
  + [2.4 Automate Secrets Management](#24-automate-secrets-management) 
    ...
    - [2.4.1 Architectural Patterns for Automated Rotation](#241-architectural-patterns-for-automated-rotation) 

      * [Example 1: Kubernetes with a Sidecar Container](#example-1-kubernetes-with-a-sidecar-container)
      * [Example 2: Serverless Function for Database Credential Rotation](#example-2-serverless-function-for-database-credential-rotation)
  ...
  + [2.12 Passwordless Authentication and Token Security](#212-passwordless-authentication-and-token-security)
...
* [5 Containers & Orchestrators](#5-containers-orchestrators) 
  ...
  + [5.2 Short-Lived Sidecar Containers](#52-short-lived-sidecar-containers)
...
* [6 Implementation Guidance](#6-implementation-guidance) 
  ...
  + [6.5 Usability and Ease of Onboarding](#65-usability-and-ease-of-onboarding)
...
# Secrets Management Cheat Sheet[¶](#secrets-management-cheat-sheet "Permanent link")

## 1 Introduction[¶](#1-introduction "Permanent link")
...
## 2 General Secrets Management[¶](#2-general-secrets-management "Permanent link")

The following sections address the main concepts relating to secrets management.

### 2.1 High Availability[¶](#21-high-availability "Permanent link")
...
### 2.2 Centralize and Standardize[¶](#22-centralize-and-standardize "Permanent link")
...
### 2.3 Access Control[¶](#23-access-control "Permanent link")
...
### 2.4 Automate Secrets Management[¶](#24-automate-secrets-management "Permanent link")
...
#### 2.4.1 Architectural Patterns for Automated Rotation[¶](#241-architectural-patterns-for-automated-rotation "Permanent link")

To illustrate how to design systems that support automated secret rotation, here are a few architectural patterns:

##### Example 1: Kubernetes with a Sidecar Container[¶](#example-1-kubernetes-with-a-sidecar-container "Permanent link")
...
##### Example 2: Serverless Function for Database Credential Rotation[¶](#example-2-serverless-function-for-database-credential-rotation "Permanent link")
...
### 2.5 Handling Secrets in Memory[¶](#25-handling-secrets-in-memory "Permanent link")
...
### 2.6 Auditing[¶](#26-auditing "Permanent link")
...
### 2.7 Secret Lifecycle[¶](#27-secret-lifecycle "Permanent link")
...
#### 2.7.1 Creation[¶](#271-creation "Permanent link")
...
#### 2.7.2 Rotation[¶](#272-rotation "Permanent link")
...
#### 2.7.3 Revocation[¶](#273-revocation "Permanent link")
...
#### 2.7.4 Expiration[¶](#274-expiration "Permanent link")
...
### 2.8 Transport Layer Security (TLS) Everywhere[¶](#28-transport-layer-security-tls-everywhere "Permanent link")
...
### 2.9 Downtime, Break-glass, Backup and Restore[¶](#29-downtime-break-glass-backup-and-restore "Permanent link")
...
### 2.10 Policies[¶](#210-policies "Permanent link")
...
### 2.11 Metadata: prepare to move the secret[¶](#211-metadata-prepare-to-move-the-secret "Permanent link")
...
### 2.12 Passwordless Authentication and Token Security[¶](#212-passwordless-authentication-and-token-security "Permanent link")
...
For more detailed guidance on securing OAuth 2.0 and OpenID Connect implementations, refer to the [OAuth2 Cheat Sheet](OAuth2_Cheat_Sheet.html).

## 3 Continuous Integration (CI) and Continuous Deployment (CD)[¶](#3-continuous-integration-ci-and-continuous-deployment-cd "Permanent link")
...
### 3.1 Hardening your CI/CD pipeline[¶](#31-hardening-your-cicd-pipeline "Permanent link")
...
### 3.2 Where should a secret be?[¶](#32-where-should-a-secret-be "Permanent link")
...
#### 3.2.1 As part of your CI/CD tooling[¶](#321-as-part-of-your-cicd-tooling "Permanent link")
...
#### 3.2.2 Storing it in a secrets management system[¶](#322-storing-it-in-a-secrets-management-system "Permanent link")
...
#### 3.2.3 Not touched by CI/CD at all[¶](#323-not-touched-by-cicd-at-all "Permanent link")
...
### 3.3 Authentication and Authorization of CI/CD tooling[¶](#33-authentication-and-authorization-of-cicd-tooling "Permanent link")
...
### 3.4 Logging and Accounting[¶](#34-logging-and-accounting "Permanent link")
...
### 3.5 Rotation vs Dynamic Creation[¶](#35-rotation-vs-dynamic-creation "Permanent link")
...
### 3.6 Pipeline Created Secrets[¶](#36-pipeline-created-secrets "Permanent link")
...
## 4 Cloud Providers[¶](#4-cloud-providers "Permanent link")
...
### 4.1 Services to Use[¶](#41-services-to-use "Permanent link")
...
#### 4.1.1 AWS[¶](#411-aws "Permanent link")
...
##### 4.1.1.1 AWS Nitro Enclaves[¶](#4111-aws-nitro-enclaves "Permanent link")
...
##### 4.1.1.2 AWS CloudHSM[¶](#4112-aws-cloudhsm "Permanent link")
...
#### 4.1.2 GCP[¶](#412-gcp "Permanent link")
...
##### 4.1.2.1 Google Cloud Confidential Computing[¶](#4121-google-cloud-confidential-computing "Permanent link")
...
#### 4.1.3 Azure[¶](#413-azure "Permanent link")
...
##### 4.1.3.1 Azure Confidential Computing[¶](#4131-azure-confidential-computing "Permanent link")
...
##### 4.1.3.2 Azure Dedicated HSM[¶](#4132-azure-dedicated-hsm "Permanent link")
...
#### 4.1.4 Other clouds, Multi-cloud, and Cloud agnostic[¶](#414-other-clouds-multi-cloud-and-cloud-agnostic "Permanent link")
...
### 4.2 Envelope & client-side encryption[¶](#42-envelope-client-side-encryption "Permanent link")

This section will describe how a secret is encrypted and how you can manage the keys for that encryption in the cloud.

#### 4.2.1 Client-side encryption versus server-side encryption[¶](#421-client-side-encryption-versus-server-side-encryption "Permanent link")
...
#### 4.2.2 Bring Your Own Key versus Cloud Provider Key[¶](#422-bring-your-own-key-versus-cloud-provider-key "Permanent link")
...
### 4.3 Identity and Access Management (IAM)[¶](#43-identity-and-access-management-iam "Permanent link")
...
### 4.4 API limits[¶](#44-api-limits "Permanent link")
...
## 5 Containers & Orchestrators[¶](#5-containers-orchestrators "Permanent link")

You can enrich containers with secrets in multiple ways: build time (not recommended) and during orchestration/deployment.

### 5.1 Injection of Secrets (file, in-memory)[¶](#51-injection-of-secrets-file-in-memory "Permanent link")
...
### 5.2 Short-Lived Sidecar Containers[¶](#52-short-lived-sidecar-containers "Permanent link")
...
### 5.3 Internal vs External Access[¶](#53-internal-vs-external-access "Permanent link")
...
When the orchestrator stores secrets (e.g., Kubernetes Secrets), make sure that the storage backend of the orchestrator is encrypted and you manage the keys well. See the [Kubernetes Security Cheat Sheet](Kubernetes_Security_Cheat_Sheet.html) for more information.

## 6 Implementation Guidance[¶](#6-implementation-guidance "Permanent link")
...
### 6.1 Key Material Management Policies[¶](#61-key-material-management-policies "Permanent link")

Key material management is discussed in the [Key Management Cheat Sheet](Key_Management_Cheat_Sheet.html)

### 6.2 Dynamic vs Static Use Cases[¶](#62-dynamic-vs-static-use-cases "Permanent link")
...
### 6.3 Ensure limitations are in place[¶](#63-ensure-limitations-are-in-place "Permanent link")
...
### 6.4 Security Event Monitoring is Key[¶](#64-security-event-monitoring-is-key "Permanent link")
...
### 6.5 Usability and Ease of Onboarding[¶](#65-usability-and-ease-of-onboarding "Permanent link")
...
## 7 Encryption[¶](#7-encryption "Permanent link")
...
### 7.1 Encryption Types to Use[¶](#71-encryption-types-to-use "Permanent link")
...
### 7.2 Convergent Encryption[¶](#72-convergent-encryption "Permanent link")
...
### 7.3 Where to store the Encryption Keys?[¶](#73-where-to-store-the-encryption-keys "Permanent link")

You should not store keys next to the secrets they encrypt, except if those keys are encrypted themselves (see envelope encryption). Start by consulting the [Key Management Cheat Sheet](Key_Management_Cheat_Sheet.html) on where and how to store the encryption and possible HMAC keys.

### 7.4 Encryption as a Service (EaaS)[¶](#74-encryption-as-a-service-eaas "Permanent link")
...
## 8 Detection[¶](#8-detection "Permanent link")
...
### 8.1 General detection approaches[¶](#81-general-detection-approaches "Permanent link")
...
### 8.2 Types of secrets to be detected[¶](#82-types-of-secrets-to-be-detected "Permanent link")
...
### 8.3 Detection lifecycle[¶](#83-detection-lifecycle "Permanent link")
...
### 8.4 Documentation for how to detect secrets[¶](#84-documentation-for-how-to-detect-secrets "Permanent link")
...
## 9 Incident Response[¶](#9-incident-response "Permanent link")
...
### 9.1 Documentation[¶](#91-documentation "Permanent link")
...
### 9.2 Remediation[¶](#92-remediation "Permanent link")
...
### 9.3 Logging[¶](#93-logging "Permanent link")
...
Consider using a standardized logging format and vocabulary such as the [Logging Vocabulary Cheat Sheet](Logging_Vocabulary_Cheat_Sheet.html) to ensure that all necessary information is logged.

## 10 Secrets Management in a Multi-Cloud Environment[¶](#10-secrets-management-in-a-multi-cloud-environment "Permanent link")

### 10.1 Introduction[¶](#101-introduction "Permanent link")
...
### 10.2 Challenges[¶](#102-challenges "Permanent link")
...
### 10.3 Best Practices[¶](#103-best-practices "Permanent link")
...
### 10.4 References[¶](#104-references "Permanent link")
...
## 11 Related Cheat Sheets & further reading[¶](#11-related-cheat-sheets-further-reading "Permanent link")

* [Key Management Cheat Sheet](Key_Management_Cheat_Sheet.html)
* [Logging Cheat Sheet](Logging_Cheat_Sheet.html)
* [Password Storage Cheat Sheet](Password_Storage_Cheat_Sheet.html)
* [Cryptographic Storage Cheat Sheet](Cryptographic_Storage_Cheat_Sheet.html)
