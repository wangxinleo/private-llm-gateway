...
python3 ./.trellis/scripts/task.py archive <name> [--outcome completed|cancelled] [--reason <text>] [--no-commit]
...
**Current-task mechanism**: `task.py create` creates the task directory and (when session identity is available) auto-sets the per-session active-task pointer so the planning breadcrumb fires immediately. `task.py start` writes the same pointer (idempotent if already set) and flips `task.json.status` from `planning` to `in_progress`. State is stored under `.trellis/.runtime/sessions/`. If no context key is available from hook input, `TRELLIS_CONTEXT_ID`, or a platform-native session environment variable, there is no active task and `task.py start` fails with a session identity hint. `task.py finish` deletes the current session file (status unchanged). `task.py archive <task>` defaults to `status=completed` with `completedAt` and `closedAt` set, or accepts `--outcome cancelled --reason <text>` to write `status=cancelled`, `completedAt=null`, `closedAt` set, and a labeled cancellation note. Parent archiving refuses while any referenced child directory is still active. Successful archives move the directory to `archive/` and delete runtime session files that still point at the archived task.
...
<!--
  ...
  TAG ↔ PHASE scoping:
   ...
   [workflow-state:completed]    → currently DEAD: cmd_archive flips
                                     ...
                                     transition)
   [workflow-state:cancelled]   → terminal cancellation outcome; normally
                                    the task is archived in the same command,
                                    but the block also covers an active task
                                    whose status was set to cancelled
...
Task is cancelled and terminal. Do not resume implementation or treat it as unfinished. Preserve the cancellation reason; if the task directory is still active, archive it with `python3 ./.trellis/scripts/task.py archive <task> --outcome cancelled --reason "<reason>"`, or start a new task.
[/workflow-state:cancelled]
...
| Cancelled terminal outcome | `[workflow-state:cancelled]` (terminal guidance for an active cancelled task) |
