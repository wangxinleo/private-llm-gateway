...
Provides:
    ...
    cmd_archive        - Archive task with an explicit outcome
...
from .task_utils import (
    ...
    find_task_by_name,
    get_archive_destination,
    resolve_task_dir,
    run_task_hooks,
)

ArchiveOutcome = Literal["completed", "cancelled"]
...
def _restore_task_metadata(task_json_path: Path, original_bytes: bytes) -> bool:
    """Restore task metadata after a failed archive move."""
    return write_bytes_atomic(task_json_path, original_bytes)


def _active_child_dirs(child_names: list[str], tasks_dir: Path) -> list[Path]:
    """Return active child directories referenced by a task."""
    active_children: list[Path] = []
    for child_name in child_names:
        child_dir = find_task_by_name(child_name, tasks_dir)
        if child_dir is not None:
            active_children.append(child_dir)
    return active_children


def _append_cancellation_reason(notes: str, reason: str) -> str:
    """Append a clearly labeled cancellation reason to task notes."""
    if not notes:
        return f"Cancellation reason: {reason}"

    separator = "\n" if notes.endswith("\n") else "\n\n"
    return f"{notes}{separator}Cancellation reason: {reason}"
...
def cmd_archive(args: argparse.Namespace) -> int:
    """Archive a task with a completed or cancelled outcome."""
    repo_root = get_repo_root()
    task_name = args.name
    raw_outcome = getattr(args, "outcome", "completed")
    ...
    if not isinstance(raw_outcome, str):
        print(colored(f"Error: Unsupported archive outcome: {raw_outcome}", Colors.RED), file=sys.stderr)
        return 1

    archive_outcomes: dict[str, ArchiveOutcome] = {
        "completed": "completed",
        "cancelled": "cancelled",
    }
    outcome = archive_outcomes.get(raw_outcome)
    if outcome is None:
        print(colored(f"Error: Unsupported archive outcome: {raw_outcome}", Colors.RED), file=sys.stderr)
        return 1
    ...
    match outcome:
        ...
        case "cancelled":
            ...
            if not normalized_reason:
                print(
                    colored(
                        "Error: --reason is required and must not be blank when --outcome is cancelled",
                        Colors.RED,
                    ),
                    file=sys.stderr,
                )
    ...
    if get_archive_destination(task_dir, repo_root) is None:
        return 1

    dir_name = task_dir.name
    task_json_path = task_dir / FILE_TASK_JSON

    if task_json_path.is_symlink() or not task_json_path.is_file():
        print(
            colored("Error: task.json is not a readable JSON object", Colors.RED),
            file=sys.stderr,
        )
        return 1

    # Refuse to archive a parent until every referenced child directory has
    # moved out of the active task set. This check must precede all writes.
    ...
    task_children: list[str] = []
    raw_children = data.get("children")
    if isinstance(raw_children, list):
        task_children = [child_name for child_name in raw_children if isinstance(child_name, str)]
    active_children = _active_child_dirs(task_children, tasks_dir)
    if active_children:
        active_names = ", ".join(child.name for child in active_children)
        print(
            colored(
                f"Error: Cannot archive parent task while active child task(s) exist: {active_names}",
                Colors.RED,
            ),
            file=sys.stderr,
        )
        print("Archive each active child task first.", file=sys.stderr)
        return 1

    task_reference = _repo_relative_path(task_dir, repo_root)
    try:
        original_bytes = task_json_path.read_bytes()
    except OSError as e:
        print(colored(f"Error: Failed to read task metadata: {e}", Colors.RED), file=sys.stderr)
        return 1

    # Update status before archiving. The JSON object is written back as-is so
    # fields unknown to this script remain intact.
    data["status"] = outcome
    ...
    if cancellation_reason is not None:
        ...
        notes = existing_notes if isinstance(existing_notes, str) else ""
        data["notes"] = _append_cancellation_reason(notes, cancellation_reason)

    if not write_json(task_json_path, data):
        print(
            colored("Error: Failed to write task metadata", Colors.RED),
            file=sys.stderr,
        )
        return 1

    # Move the task before clearing sessions so a failed move leaves pointers
    # intact. The pre-move reference matches the value stored in session files.
    result = archive_task_complete(task_dir, repo_root)
    if "archived_to" not in result:
        if not _restore_task_metadata(task_json_path, original_bytes):
            print(
                colored("Error: Failed to restore original task metadata", Colors.RED),
                file=sys.stderr,
            )
        return 1
    ...
    try:
        clear_task_from_sessions(task_reference, repo_root)
    except OSError as e:
        print(colored(f"Error: Failed to clear active task sessions: {e}", Colors.RED), file=sys.stderr)
        return 1
