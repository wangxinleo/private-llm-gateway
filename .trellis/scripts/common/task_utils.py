...
def _has_symlink_component(path: Path, root: Path) -> bool:
    """Return whether an existing component between root and path is a symlink."""
    try:
...
def get_archive_destination(
    task_dir_abs: Path,
    repo_root: Path | None = None,
) -> Path | None:
    """Validate an active task and return its archive destination.

    This performs no filesystem mutation so callers can run it before writing
    task metadata.
    """
    root = _absolute_path(repo_root or get_repo_root())
    tasks_dir = _absolute_path(get_tasks_dir(root))
    task_dir = _absolute_path(task_dir_abs)

    if not tasks_dir.is_dir() or _has_symlink_component(tasks_dir, root):
        print(f"Error: invalid task root: {tasks_dir}", file=sys.stderr)
        return None

    if task_dir.parent != tasks_dir or task_dir.name == DIR_ARCHIVE:
        print(f"Error: task path must be a direct active task: {task_dir_abs}", file=sys.stderr)
        return None

    if not task_dir.is_dir() or task_dir.is_symlink():
        print(f"Error: task directory is missing or symlinked: {task_dir_abs}", file=sys.stderr)
        return None

    archive_dir = tasks_dir / DIR_ARCHIVE
    if archive_dir.exists() and not archive_dir.is_dir():
        print(f"Error: archive path is not a directory: {archive_dir}", file=sys.stderr)
        return None
    if _has_symlink_component(archive_dir, root):
        print(f"Error: archive path contains a symlink: {archive_dir}", file=sys.stderr)
        return None

    year_month = datetime.now().strftime("%Y-%m")
    month_dir = archive_dir / year_month
    if month_dir.exists() and not month_dir.is_dir():
        print(f"Error: archive month path is not a directory: {month_dir}", file=sys.stderr)
        return None
    if _has_symlink_component(month_dir, root):
        print(f"Error: archive month path contains a symlink: {month_dir}", file=sys.stderr)
        return None

    destination = month_dir / task_dir.name
...
def archive_task_dir(task_dir_abs: Path, repo_root: Path | None = None) -> Path | None:
    ...
    """
    destination = get_archive_destination(task_dir_abs, repo_root)
    if destination is None:
        return None

    month_dir = destination.parent

    # Create archive directory
    try:
        month_dir.mkdir(parents=True, exist_ok=True)
        if month_dir.is_symlink() or not month_dir.is_dir():
            raise OSError(f"archive month path is not a directory: {month_dir}")
    except (OSError, IOError) as e:
        print(f"Error: Failed to create archive directory: {e}", file=sys.stderr)
        return None

    # Recheck immediately before the move so a concurrent destination cannot
    # turn shutil.move into an unintended nested directory.
    if destination.exists() or destination.is_symlink():
        print(f"Error: archive destination already exists: {destination}", file=sys.stderr)
        return None

    try:
        shutil.move(str(_absolute_path(task_dir_abs)), str(destination))
