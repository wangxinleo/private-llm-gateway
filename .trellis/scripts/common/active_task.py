...
def _comparison_task_path(task_path: str, repo_root: Path) -> Path | None:
    """Normalize a task reference without requiring its directory to exist."""
    normalized = normalize_task_ref(task_path)
    if not normalized:
        return None
    full_path = resolve_task_ref(normalized, repo_root)
    if full_path is None:
        return None
    return Path(os.path.abspath(os.fspath(full_path)))
...
def clear_task_from_sessions(task_path: str, repo_root: Path) -> int:
    ...
    target = _comparison_task_path(task_path, repo_root)
    if not target:
        return 0
    ...
    for session_path in sessions_dir.glob("*.json"):
        ...
        if not current:
            continue
        current_ref = _comparison_task_path(current, repo_root)
        if current_ref != target:
            continue
        if session_path.is_file() and _remove_file(session_path):
            cleared += 1
        elif session_path.exists():
            failures.append(session_path)

    if failures:
        failed_paths = ", ".join(str(path) for path in failures)
        raise OSError(f"unable to remove session files: {failed_paths}")
