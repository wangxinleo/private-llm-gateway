...
Usage:
    ...
    python3 task.py archive <task-dir> [--outcome completed|cancelled] [--reason <text>] [--no-commit]  # Archive task
...
Examples:
  ...
  python3 task.py archive add-login --outcome completed
  python3 task.py archive abandoned-task --outcome cancelled --reason "Scope was withdrawn"
...
def main() -> int:
    ...
    p_archive.add_argument(
        ...
        default="completed",
        help="Archive outcome (default: completed)",
    )
    p_archive.add_argument(
        "--reason",
        help="Cancellation reason (required when outcome is cancelled)",
    )
