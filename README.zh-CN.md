...
后台设置页还可以管理写入 SQLite 的热更新配置：扫描体积阈值、上下文密钥限制、bypass 路径选项和高风险资产白名单。
...
所有分级运行相同的窗口扫描器：PII 全文扫描、全局高信号层（provider token 前缀）、以及白名单高风险资产与敏感键名上下文窗口扫描。分级值目前仅影响审计分类与调试日志；阈值和分块大小仍可在后台设置页修改，用于审计展示。
...
- Settings：热更新扫描阈值、路径前缀选项和高风险资产白名单。
...
| `src/scanner/` | 隐私扫描管线：密钥、上下文 key、PII、文件名、multipart 解析和高风险资产窗口扫描。 |
